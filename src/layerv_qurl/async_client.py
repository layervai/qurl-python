...
if TYPE_CHECKING:
    # The `list()` method on AsyncQURLClient shadows the `list` type
    # inside the class body, so parameter/return annotations that need
    # the builtin must reference `builtins.list[...]` explicitly. The
...
class AsyncQURLClient:
    ...
    def __repr__(self) -> str:
        ...
        return f"AsyncQURLClient(api_key={api_key!r}, base_url='{self._base_url}')"
    ...
    async def regenerate_domain_token(
        self, domain: str, *, idempotency_key: str | None = None
    ) -> Domain:
    ...
    async def create_api_key(
        ...
        *,
        ...
        ``scopes``. ``kind="enrollment_token"`` mints a one-shot (<=24h)
        Connector/agent enrollment credential: pass ``target`` and
        ``claims`` instead of ``scopes`` (the server assigns them).
